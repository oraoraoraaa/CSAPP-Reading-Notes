...
/* The code shows what happens by passing the int value to the signed version of
   showBytes. */
{
  unsigned long i = 0;
  for (i = 0; i < len; i++) {
...
void showBytesSigned(char *start, size_t len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    /* The integer promotion uses signed extension of course, which kind of like
     * doing an arithmetic right shift. */
    /* If passed 0x80 which is 0b10000000, the printf would promote it to
     * 0xffffff80. */
  }
